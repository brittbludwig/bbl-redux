  ...
  return (
    <div className="bg-white fixed top-0 left-0 w-full h-15 lg:h-20 z-50">
      <div className="flex items-center justify-between h-full px-6">
        ...
        <button onClick={toggleNav} className="lg:hidden flex justify-end py-3">
          <div>
        ...
        <div className="relative py-4">
          <div className={`
            absolute right-0 top-0 text-right
            hidden lg:block lg:relative lg:w-full
            ${isNavOpen ? 'block! bg-white px-8 py-4 md:px-0' : ''}
          `}>
            <ul className="list-none m-0 p-0">
              {pageSections.map((section, index) => index > 0 && (
                <li key={section.id} className="block lg:inline-block mx-3">
                  <a
                    href={`#${section.id}`}
                    style={{ color: activeColor }}
                    className="text-[2rem] lg:text-xl block transition-all duration-200 cursor-pointer hover:no-underline"
                  >
                    {section.header}
                  </a>
                </li>
